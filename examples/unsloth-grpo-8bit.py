...
# ==================== NPU torch.compile 配置 ====================
# 关键：设置 UNSLOTH_FULLGRAPH=0，让 unsloth 生成的编译代码使用 fullgraph=False
# 这可以避免 torchair 在 AOT Autograd 分区时的兼容性问题
...
# 创建 NPU 编译配置
npu_config = CompilerConfig()
# 可选：配置调试选项
# npu_config.debug.graph_dump.type = "pbtxt"
...
def npu_torch_compile(model=None, *, backend=None, **kwargs):
    """NPU 版本的 torch.compile，自动使用 torchair 后端"""
    # 强制使用 npu_backend
    backend = npu_backend
    
    # 移除 NPU/torchair 不支持的参数
    kwargs.pop("options", None)
    
    # 强制 fullgraph=False 以提高兼容性
    kwargs["fullgraph"] = False
    
    # dynamic=True 对于变长输入是必要的
    kwargs.setdefault("dynamic", True)
    
    func_name = getattr(model, '__name__', type(model).__name__) if model else 'None'
    print(f"torch.compile : {func_name}")

    return _original_torch_compile(model, backend=backend, **kwargs)

torch.compile = npu_torch_compile
print("✅ 已配置 torchair 作为 torch.compile 后端 (fullgraph=False)")
